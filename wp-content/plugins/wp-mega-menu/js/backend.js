  ...
  /* arrow type preview */
   ...
   /* check of follow_scroll div exists on page or not for visibility of save custom theme on page scroll */
    if ($('.follow-scroll').length > 0) {
      ...
      $('select.wpmm_theme_type').on('change',function(){
         var selected_value =  this.value;
         if(selected_value == "available_skins"){
          $('tr.wpmm_show_themes').hide();
          $('tr.wpmm_show_skins').show('slow');
         }else{
          $('tr.wpmm_show_themes').show('slow');
          $('tr.wpmm_show_skins').hide();
         }
      });
    ...
    var selectedval =  $('select.wpmm_theme_type option:selected').val();
